        ...
        <b-card no-body>
          <b-card-body>
            <div class="d-flex flex-wrap justify-content-between gap-3">
              ...
              <div>
                <a href="#!" class="btn btn-primary d-inline-flex align-items-center"> <i class="bx bx-plus me-1"></i>Create Task </a>
              </div>
            </div>
          </b-card-body>
          <div>
            <b-table-simple responsive class="text-nowrap table-centered mb-0">
              ...
              <b-tbody>
                <b-tr v-for="(todo, idx) in todosData" :key="idx">
                  <b-td>
                    <div class="d-flex align-items-center gap-2">
                      <div class="form-check form-todo ps-4">
                        <input type="checkbox" class="form-check-input rounded-circle mt-0 fs-18" :id="`customCheck${todo.id}`" :checked="todo.checked" />
                  ...
                  <b-td>
                    <b-badge :variant="null" :class="todo.status === 'completed' ? 'badge-soft-success' : todo.status === 'new' ? 'badge-soft-info' : todo.status === 'pending' ? 'badge-soft-primary' : 'badge-soft-warning'">
                      {{ kebabToTitleCase(todo.status) }}
                    </b-badge>
                  </b-td>
                  <b-td :class="todo.priority === 'High' ? 'text-danger' : todo.priority === 'Medium' ? 'text-warning' : 'text-success'"> <i class="bx bxs-circle me-1"></i>{{ todo.priority }} </b-td>
                  <b-td>
            ...
            <div class="align-items-center justify-content-between row g-0 text-center text-sm-start p-3 border-top">
              ...
              <div class="col-sm-auto mt-3 mt-sm-0">
                <b-pagination class="m-0" pills v-model="currentPage" :per-page="perPageItem" :total-rows="todosData.length" />
              </div>
            </div>
          </div>
        </b-card>
