<template>
  <DefaultLayout>
    ...
    <b-row>
      <b-col xl="9">
        ...
        <UIComponentCard title="Scrolling Long Content" id="scrolling-long-content" caption="When modals become too long for the user&rsquo;s viewport or device, they scroll independent of the page itself. Try the demo below to see what we mean.">
          ...
          <b-button type="button" variant="primary" class="me-2" @click="scrollingLongModal = !scrollingLongModal"> Launch demo modal </b-button>
        ...
        <UIComponentCard title="Modal Position" id="modal-position">
          <div class="d-flex flex-wrap gap-2 mt-3">
            <!-- Button trigger modal -->
            <b-button variant="primary" @click="verticalCenteredModal = !verticalCenteredModal"> Vertically centered modal </b-button>

            <!-- Button trigger modal -->
            <b-button variant="primary" @click="verticalCenteredScrollableModal = !verticalCenteredScrollableModal"> Vertically centered scrollable modal </b-button>
          </div>
          ...
          <div class="d-flex flex-wrap gap-2 mt-3">
            ...
            <div class="hstack gap-2">
              ...
              <b-button variant="success" @click="bottomModal = !bottomModal"> Bottom Modal </b-button>
            </div>
          </div>
        </UIComponentCard>
    ...
    <b-modal v-model="scrollingLongModal" title="Modal title" ok-title="Save changes" cancel-title="Close" :bodyScrolling="true">
      <div style="min-height: 1500px">
        <p>This is some placeholder content to show the scrolling behavior for modals. Instead of repeating the text the modal, we use an inline style set a minimum height, thereby extending the length of the overall modal and demonstrating the overflow scrolling. When content becomes longer than the height of the viewport, scrolling will move the modal as needed.</p>
      </div>
    </b-modal>

    <b-modal v-model="scrollingLongModal1" title="Modal title" ok-title="Save changes" cancel-title="Close" :bodyScrolling="true" :scrollable="true">
    ...
    <b-modal v-model="verticalCenteredScrollableModal" title="Modal title" ok-title="Save changes" cancel-title="Close" centered>
    ...
    <!-- Top Modal -->
    <b-modal v-model="topModal" title="Modal Heading" ok-title="Save changes" cancel-title="Close" dialog-class="modal-top">
      <h6>Text in a modal</h6>
      <p class="mb-0">Duis mollis, est non commodo luctus, nisi erat porttitor ligula.</p>
    </b-modal>

    <!-- Bottom Modal -->
    <b-modal v-model="bottomModal" title="Modal Heading" ok-title="Save changes" cancel-title="Close" dialog-class="modal-bottom">
      <h6>Text in a modal</h6>
      <p class="mb-0">Duis mollis, est non commodo luctus, nisi erat porttitor ligula.</p>
    </b-modal>
    ...
    <!-- First Modal -->
    <b-modal v-model="toggleModal1" title="Modal 1" @ok="toggleModal" ok-title="Open second modal" ok-only centered> Show a second modal and hide this one with the button below. </b-modal>

    <!-- Second Modal -->
    <b-modal v-model="toggleModal2" title="Modal 2" ok-title="Back to first" cancel-title="Close" @ok="toggleModal" centered> Hide this modal and show the first with the button below. </b-modal>
    ...
    <!-- Fullscreen Modal -->
    <b-modal v-model="fsModal" title="Full screen modal" ok-only ok-title="Close" ok-variant="secondary" fullscreen> ... </b-modal>
    <b-modal v-model="fsBSMModal" title="Full screen below sm" ok-only ok-title="Close" ok-variant="secondary" fullscreen="sm"> ... </b-modal>
    <b-modal v-model="fsBMDModal" title="Full screen below md" ok-only ok-title="Close" ok-variant="secondary" fullscreen="md"> ... </b-modal>
    <b-modal v-model="fsBLGModal" title="Full screen below lg" ok-only ok-title="Close" ok-variant="secondary" fullscreen="lg"> ... </b-modal>
    <b-modal v-model="fsBXLModal" title="Full screen below xl" ok-only ok-title="Close" ok-variant="secondary" fullscreen="xl"> ... </b-modal>
    <b-modal v-model="fsBXXLModal" title="Full screen below xxl" ok-only ok-title="Close" ok-variant="secondary" fullscreen="xxl"> ... </b-modal>
    ...
    <b-modal v-model="primaryAlertModal" hide-header hide-footer size="sm" content-class="bg-primary">
      <div class="text-center">
        ...
        <b-button type="button" variant="light" class="mt-3" @click="primaryAlertModal = !primaryAlertModal"> Continue </b-button>
      </div>
    </b-modal>
    ...
    <b-modal v-model="secondaryAlertModal" hide-header hide-footer size="sm" content-class="modal-filled bg-secondary">
      <div class="text-center">
        ...
        <b-button type="button" variant="light" class="mt-3" @click="secondaryAlertModal = !secondaryAlertModal"> Continue </b-button>
      </div>
    </b-modal>
    ...
    <b-modal v-model="successAlertModal" hide-header hide-footer size="sm" content-class="modal-filled bg-success">
      <div class="text-center">
        ...
        <h4 class="mt-3 text-white">Well Done!</h4>
        <p class="mt-3">Cras mattis consectetur purus sit amet fermentum. Cras justo odio, dapibus ac facilisis in, egestas eget quam.</p>
        <b-button type="button" variant="light" class="mt-3" @click="successAlertModal = !successAlertModal"> Continue </b-button>
      </div>
    </b-modal>
    ...
    <b-modal v-model="infoAlertModal" hide-header hide-footer size="sm" content-class="modal-filled bg-info">
      <div class="text-center">
        ...
        <h4 class="mt-3 text-white">Well Done!</h4>
        <p class="mt-3">Cras mattis consectetur purus sit amet fermentum. Cras justo odio, dapibus ac facilisis in, egestas eget quam.</p>
        <b-button type="button" variant="light" class="mt-3" @click="infoAlertModal = !infoAlertModal"> Continue </b-button>
      </div>
    </b-modal>
  </DefaultLayout>
</template>
