<template>
  <DefaultLayout>
    <b-row class="g-1">
      <b-col xxl="3">
        ...
        <div class="d-block d-xl-none">
          <b-offcanvas v-model="contactOffcanvas" class="offcanvas-xxl h-100" placement="start" body-class="p-0 h-100" no-header>
            <ContactOffcanvas />
          </b-offcanvas>
        </div>
      </b-col>
      ...
      <b-col xxl="9">
        <b-card no-body class="position-relative overflow-hidden">
          <b-card-header class="d-flex align-items-center mh-100">
            <b-button variant="light" class="d-xxl-none d-flex align-items-center px-2 me-2" type="button" @click="contactOffcanvas = !contactOffcanvas">
              ...
              <template v-for="(content, idx) in chatMessages" :key="idx">
                <li class="clearfix" :class="content.isSender && 'odd'">
                  <div class="chat-conversation-text ms-0" v-for="(msg, idx) in content.msg" :key="idx">
                      ...
                      <div class="d-flex justify-content-end">
                        <DropDown customClass="chat-conversation-actions dropstart">
                          ...
                          <div class="dropdown-menu">
                            <a class="dropdown-item" href="javascript: void(0);" v-for="(item, idx) in dropdownItem" :key="idx">
                    ...
                    <p class="text-muted fs-12 mb-0 mt-1" :class="!content.isSender && 'ms-2'" v-if="content.timeStamp && content.msg.length - 1 === idx">{{ content.timeStamp }}<i class="bx bx-check-double ms-1 text-primary" v-if="content.isRead"></i></p>
                  </div>
                </li>
              </template>
            </simplebar>
                ...
                <b-row class="align-items-center">
                  <div class="col mb-2 mb-sm-0 d-flex">
                    <div class="input-group">
                      <a href="javascript: void(0);" class="btn btn-sm btn-light d-flex align-items-center input-group-text">
                        <i class="bx bx-smile fs-18"></i>
                      </a>
                      <input type="text" class="form-control border-0" placeholder="Enter your message" v-model="messageState.message" />
                    </div>
                  </div>
    ...
    <!-- Voice Call Modal -->
    <b-modal v-model="voiceModal" size="sm" content-class="voice-call" body-class="pt-0 text-center" header-class="border-0 mt-5 justify-content-center" hide-footer centered>
      <template #header>
    ...
    <!-- Video Call Modal -->
    <b-modal v-model="videoModal" size="sm" content-class="video-call" header-class="border-0 mb-5 justify-content-end" hide-footer centered>
      <template #header>
