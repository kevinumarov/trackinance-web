    ...
    <b-card-body class="p-0 pb-3 ps-2">
      <simplebar class="p-3" data-simplebar style="max-height: 386px">
        <b-form-group class="form-todo">
          <div v-for="(option, idx) in options" :key="option.value" :class="{ 'my-3': idx !== 0 && idx !== options.length - 1 }">
            <b-form-checkbox size="md" v-model="selected" :value="option.value" class="rounded-circle fs-18">
              {{ option.text }}
            </b-form-checkbox>
          </div>
        </b-form-group>
